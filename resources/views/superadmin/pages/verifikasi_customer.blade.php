...
<div class="space-y-6 font-['Urbanist']">
    ...
    <div class="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm">
        ...
        <div class="flex items-center justify-between gap-4">
            <div class="text-[16px] font-semibold text-slate-900">Data Customer</div>

            <div class="flex items-center gap-3 relative z-50">

                {{-- SEARCH (GET) --}}
                <form method="GET" action="{{ route('sa.verifikasi.customer') }}" class="relative w-[260px]">
                ...
                {{-- DOWNLOAD --}}
                <a id="btnDownload"
                   href="{{ route('sa.verifikasi.customer.download', request()->query()) }}"
                ...
                @else
                    @foreach($rows as $c)
                        ...
                        <tr class="text-slate-700">
                            <td class="px-2 py-[6px] whitespace-nowrap">{{ $c->id ?? '—' }}</td>
                            ...
                            <td class="px-2 py-[6px]">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('sa.verifikasi.customer.detail', ['id' => $c->id]) }}"
                                    ...
                                    <button type="button"
                                            ...
                                            title="Block/Unblock"
                                            onclick="openBlockConfirm('{{ route('sa.verifikasi.customer.block', ['id' => $c->id]) }}')">
                                        <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none">
...
{{-- MODAL & SCRIPT block sama persis --}}
...
<form id="blockActionForm" method="POST" class="hidden">@csrf</form>
...
<script>
    ...
    function openBlockSuccess() { modalSuccess.classList.remove('hidden'); }
    function closeBlockSuccess() { modalSuccess.classList.add('hidden'); }
