...
@section('title', 'Detail Pencairan Pos Mitra')
...
<div class="max-w-6xl mx-auto space-y-8">
    ...
    <div class="bg-blue-100 rounded-2xl p-6 flex justify-between items-center">
        <div class="flex items-center gap-4">
            <img src="https://ui-avatars.com/api/?name={{ urlencode($withdrawal->partnerPost->user->name) }}&size=100&background=3b82f6&color=fff"
                 class="w-20 h-20 rounded-full object-cover border-4 border-white shadow-lg">

            <div>
                <h2 class="text-lg font-semibold">
                    {{ $withdrawal->partnerPost->user->name }}
                </h2>

                <p class="text-sm text-gray-600">
                    {{ $withdrawal->partnerPost->terminal_name }}
                </p>
        ...
        <a href="{{ route('withdrawals.partner-post.index') }}" class="px-4 py-2 bg-blue-200 hover:bg-blue-300 rounded-full text-sm transition">
            Kembali
        </a>
    </div>
    ...
    <div class="bg-white rounded-2xl p-6 border border-gray-200">
        <h3 class="text-lg font-semibold mb-6">Informasi Terminal</h3>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">

            <div>
                <label class="text-sm text-gray-600 mb-2 block">Nama Terminal</label>
                <input type="text" disabled
                       value="{{ $withdrawal->partnerPost->terminal_name }}"
                       class="w-full px-4 py-2.5 bg-gray-100 rounded-lg border border-gray-200 text-sm">
            </div>

            <div>
                <label class="text-sm text-gray-600 mb-2 block">Pemilik</label>
                <input type="text" disabled
                       value="{{ $withdrawal->partnerPost->id_fullname }}"
                       class="w-full px-4 py-2.5 bg-gray-100 rounded-lg border border-gray-200 text-sm">
            </div>
            ...
            <div>
                ...
                <input type="text" disabled
                       value="{{ $withdrawal->partnerPost->user->email }}"
                       class="w-full px-4 py-2.5 bg-gray-100 rounded-lg border border-gray-200 text-sm">
            </div>
            ...
            <div>
                ...
                <input type="text" disabled
                       value="{{ $withdrawal->partnerPost->user->phone_number ?? '-' }}"
                       class="w-full px-4 py-2.5 bg-gray-100 rounded-lg border border-gray-200 text-sm">
            </div>

            <div class="md:col-span-2">
                <label class="text-sm text-gray-600 mb-2 block">Alamat Terminal</label>
                <textarea disabled rows="2"
                       class="w-full px-4 py-2.5 bg-gray-100 rounded-lg border border-gray-200 text-sm resize-none">{{ $withdrawal->partnerPost->terminal_address }}</textarea>
            </div>

            <div>
                <label class="text-sm text-gray-600 mb-2 block">Provinsi</label>
                <input type="text" disabled
                       value="{{ $withdrawal->partnerPost->province->name ?? '-' }}"
                       class="w-full px-4 py-2.5 bg-gray-100 rounded-lg border border-gray-200 text-sm">
            </div>

            <div>
                <label class="text-sm text-gray-600 mb-2 block">Kabupaten/Kota</label>
                <input type="text" disabled
                       value="{{ $withdrawal->partnerPost->regency->name ?? '-' }}"
                       class="w-full px-4 py-2.5 bg-gray-100 rounded-lg border border-gray-200 text-sm">
            </div>
            ...
            <div>
                ...
                <input type="text" disabled
                       value="Rp {{ number_format((float) ($withdrawal->partnerPost->balance ?? 0), 0, ',', '.') }}"
                       class="w-full px-4 py-2.5 bg-gray-100 rounded-lg border border-gray-200 text-sm">
            </div>
    ...
    <div class="bg-white rounded-2xl max-w-md w-full mx-4 overflow-hidden">
        <div class="p-6">
            ...
            <p class="text-gray-600 mb-6">
                ...
                untuk terminal <span class="font-semibold">{{ $withdrawal->partnerPost->terminal_name }}</span>.
            </p>
            ...
            <form action="{{ route('withdrawals.partner-post.update', $withdrawal->id) }}" method="POST">
                @csrf
                @method('PUT')
